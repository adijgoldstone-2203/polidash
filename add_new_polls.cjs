...
// Find existing poll IDs and max ID
...
console.log(`Existing Poll IDs count in src/polls.ts: ${existingIds.size}`);
console.log(`Max Poll ID in src/polls.ts: ${maxId}`);
...
for (let i = 1; i < rows.length; i++) {
  ...
  // Only include polls newer than maxId in existing DB
  if (!id || existingIds.has(id) || isNaN(numId) || numId <= maxId) {
    continue;
  }
...
// Sort newPolls descending by ID (numeric comparison)
newPolls.sort((a, b) => parseInt(b.id, 10) - parseInt(a.id, 10));

console.log(`Found ${newPolls.length} NEW poll(s) newer than ID ${maxId}:`);
console.dir(newPolls, { depth: null });

if (newPolls.length === 0) {
  console.log('No new polls to insert. src/polls.ts is up to date.');
  process.exit(0);
}

// Format TS string for insertion
let formattedTs = '';
newPolls.forEach(p => {
  formattedTs += `  {\n`;
  formattedTs += `    id: "${p.id}",\n`;
...
pollsTsContent = pollsTsContent.slice(0, insertIndex + targetMarker.length) + formattedTs + pollsTsContent.slice(insertIndex + targetMarker.length);

fs.writeFileSync('src/polls.ts', pollsTsContent, 'utf8');
console.log(`Successfully updated src/polls.ts with ${newPolls.length} new poll(s)! Added IDs: ${newPolls.map(p => p.id).join(', ')}`);
